...
> **Pendiente de publicar.** Pegar aquí la URL una vez subido.

| Campo | Valor |
|---|---|
| URL | _(pendiente)_ |
| Duración | _(pendiente)_ |
| Grabado el | _(pendiente)_ |
| Commit del repositorio que se demuestra | _(pendiente)_ |
...
El guion completo —qué se enseña en pantalla, qué se dice encima y en qué orden
sacrificar bloques si hay que recortar— está en
[`../guion-video.md`](../guion-video.md).

## Por qué el archivo no está en el repositorio

Un MP4 de varios minutos supera el límite de 100 MB por archivo de GitHub y ensucia
el historial para siempre. El video se aloja fuera y aquí queda el enlace; los `.mp4`
y `.mov` de esta carpeta están en `.gitignore`.
